...
confdir="/tank/etc"
conffile="$confdir/zrep-lxd.conf"
...
f_zrep(){
	...
	if ! zfs list -o name tank/zrep-lxd/$virttype/$s_host > /dev/null 2>&1;
		then
			zfs create tank/zrep-lxd/$virttype/$s_host
			zfs create tank/zrep-lxd/$virttype/$s_host/$vm
		else
			if ! zfs list -o name tank/zrep-lxd/$virttype/$s_host/$vm > /dev/null 2>&1;
				then
					zfs create tank/zrep-lxd/$virttype/$s_host/$vm
			fi
	fi
